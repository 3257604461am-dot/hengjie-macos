...
func runChecks() throws {
    ...
    do {
        ...
        try expect(ClipboardHistoryRules.matches(searchText: "Hello SnapWeave テスト", query: "hello SnapWeave"), "中英日搜索规范化错误")
        try expect(ClipboardHistoryRules.matches(searchText: "ＡＢＣ １２３", query: "abc 123"), "全角半角搜索错误")
        try expect(!ClipboardHistoryRules.matches(searchText: "SnapWeave截图", query: "SnapWeave 翻译"), "多关键词搜索必须全部命中")
    }
    do {
    ...
    do {
        ...
        try expect(AppStoragePaths.prepare(in: support) == .notNeeded, "无旧数据时不应执行品牌迁移")
        ...
        try expect(AppStoragePaths.prepare(in: support) == .migrated, "旧数据目录迁移失败")
        let migratedPayload = AppStoragePaths.root(in: support).appendingPathComponent("payload.txt")
        try expect(FileManager.default.fileExists(atPath: migratedPayload.path), "迁移后历史载荷丢失")
    }
    do {
        let support = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: support) }
        ...
        try FileManager.default.createDirectory(at: legacy, withIntermediateDirectories: true)
        try expect(AppStoragePaths.prepare(in: support) == .bothPresent, "新旧目录并存状态识别失败")
        try expect(
            FileManager.default.fileExists(atPath: current.path) && FileManager.default.fileExists(atPath: legacy.path),
            "新旧目录并存时不得删除或合并数据"
        )
    }
}
