...
final class MenuPanelController: NSViewController, NSPopoverDelegate {
    ...
    init(actions: Actions) {
        ...
        preferredContentSize = CGSize(width: 348, height: 440)
    }
    ...
    override func loadView() {
        ...
        let actionRows = NSStackView(views: [
            row(actionButton("普通截图", symbol: "viewfinder", action: #selector(standard)), actionButton("上下长截图", symbol: "arrow.up.and.down", action: #selector(vertical))),
            row(actionButton("左右长截图", symbol: "arrow.left.and.right", action: #selector(horizontal)), actionButton("钉住区域", symbol: "pin", action: #selector(pin))),
        ...
        [captureStatus, accessibilityStatus, historyStatus].forEach {
            $0.font = .systemFont(ofSize: 11)
            $0.textColor = .secondaryLabelColor
        }
        let statuses = NSStackView(views: [captureStatus, accessibilityStatus, historyStatus])
