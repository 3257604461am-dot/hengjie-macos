...
final class DiagnosticLogger: @unchecked Sendable {
    ...
    private let ioQueue = DispatchQueue(label: "com.wonderlab.snapweave.diagnostics", qos: .utility)
    private var handle: FileHandle?

    private init() {
        directory = AppStoragePaths.root.appendingPathComponent("Diagnostics", isDirectory: true)
...
enum DiagnosticBundleExporter {
    static func present() {
        ...
        alert.informativeText = "诊断包包含 SnapWeave 运行日志、系统和权限信息，以及最近的 SnapWeave、横截或 HengJie 崩溃报告。不会包含截图、GIF、OCR/翻译文字、剪贴板内容或快捷键配置，也不会自动上传。"
        ...
        panel.nameFieldStringValue = "SnapWeave-诊断-\(timestamp()).zip"
    ...
    private static func export(to destination: URL, summary: [String: Any], logsDirectory: URL) async throws {
        try await Task.detached(priority: .utility) {
            let manager = FileManager.default
            let staging = manager.temporaryDirectory.appendingPathComponent("SnapWeave-Diagnostics-\(UUID().uuidString)", isDirectory: true)
            ...
            if let reports = try? manager.contentsOfDirectory(at: crashSource, includingPropertiesForKeys: [.contentModificationDateKey]) {
                let cutoff = Date().addingTimeInterval(-7 * 24 * 60 * 60)
                for report in reports where crashReportBelongsToApp(report.lastPathComponent) {
            ...
            if copiedCrashes == 0 {
                try Data("最近 7 天没有可读取的 SnapWeave、横截或 HengJie 崩溃报告。\n".utf8)
            ...
            guard process.terminationStatus == 0 else {
                throw NSError(domain: "SnapWeaveDiagnostics", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: "无法生成诊断 ZIP。"])
            }
        }.value
    }
    ...
    nonisolated private static func crashReportBelongsToApp(_ name: String) -> Bool {
        ["SnapWeave", "HengJie", "横截"].contains {
